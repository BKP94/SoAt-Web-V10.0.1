#!/usr/bin/env bash
# Migrate one legacy rcAdmin report folder -> scReport/Reports/rcAdmin/<menu>
# + copy its repQuery XML tree. Same transform rules as migrate_rcaccount.sh
# (8 net10/DX25.2 compat sed rules + namespace/literal rewrite rcAdmin.->scReport.Reports.rcAdmin.)
# SQL Oracle->PG migrated lazily per report (function-first sweep, phase 2).
set -euo pipefail

MENU="$1"
SRC="C:/SoAt_PEAN/rcAdmin/$MENU"
DST="C:/Project_SoAt/SoAt_WebApp/scReport/Reports/rcAdmin/$MENU"
RQ_SRC="C:/SoAt_PEAN/repQuery/rcAdmin/$MENU"
RQ_DST="C:/Project_SoAt/SoAt_WebApp/scReport/repQuery/rcAdmin/$MENU"
...
for f in "$SRC"/*.cs; do
  ...
  base=$(basename "$f")
  out="$DST/$base"
  sed -e 's/namespace rcAdmin\./namespace scReport.Reports.rcAdmin./' \
      -e 's/"rcAdmin\./"scReport.Reports.rcAdmin./g' \
